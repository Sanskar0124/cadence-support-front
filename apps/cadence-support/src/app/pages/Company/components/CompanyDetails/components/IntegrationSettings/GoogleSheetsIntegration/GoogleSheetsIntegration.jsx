...
import styles from './GoogleSheetsIntegration.module.scss';
...
// Google sheets + Excel integration ==sheets
...
export default function SalesforceIntegrationV2({ companyID }) {
  ...
  const [selectedTab, setSelectedTab] = useState({
    title: 'Sheets Set-up',
    component: <GoogleSheetsSettings companyID={companyID} />,
  });
  ...
  return (
    <div className={styles.salesforceIntegration}>
      <div className={styles.header} id="tabscontainer">
        {getSettingpageTabOptions(companyItem?.integration_type)
          ?.filter((item) => item?.title !== 'Field mapping')
          .map((tab) => (
            <div
              className={
