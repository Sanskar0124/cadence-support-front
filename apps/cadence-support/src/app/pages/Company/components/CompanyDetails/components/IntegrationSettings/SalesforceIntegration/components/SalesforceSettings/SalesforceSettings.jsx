...
import { Div, ProgressiveImg, Skeleton } from '@cadence-support/components';
import { RoundedTick, CopyBlank } from '@cadence-support/icons';

function SalesforceSettings({ companyID }) {
      ...
      {isSalesforceSettingsLoading ? (
        ...
        <div className={styles.salesforceSettings}>
          ...
          <div className={`${styles.rowValue} ${styles.connected}`}>
            {!salesforceSettings?.User?.Salesforce_Token?.is_logged_out ? (
              <>
                <h2>
                ...
                <h3>
                  Activated on{' '}
                  {moment(
                    salesforceSettings?.User?.Salesforce_Token?.created_at
                  ).format('DD MMM YYYY')}
                </h3>
              </>
            ) : (
          ...
          <div
            className={`${styles.rowValue} ${
              salesforceSettings?.User?.Salesforce_Token?.instance_url
                ? styles.url
                : ''
            }`}
          >
          ...
          <div className={styles.rowTitle}>
            <h1>Salesforce administrator</h1>
          </div>
