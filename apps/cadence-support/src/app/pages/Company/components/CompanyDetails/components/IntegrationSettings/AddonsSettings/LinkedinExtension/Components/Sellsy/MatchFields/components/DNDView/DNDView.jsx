...
import SellsyFields from './components/SellsyFields/SellsyFields';
...
const DNDView = ({
  ...
  const fetchSFFields = () => {
    ...
    companyFields(VIEWS.COMPANY, {
      onSuccess: (sfFieldsFromServer) => {
        setDefaultFieldsWhileOnboarding(sfFieldsFromServer, VIEWS.COMPANY);
        setOriginalSFFields((prev) => ({
          ...prev,
          [VIEWS.CONTACT]: sfFieldsFromServer
            ?.sort((a, b) => a.label.localeCompare(b.label))
      ...
      onError: () => addError('Make sure you have signed in with sellsy'),
    });
  };
