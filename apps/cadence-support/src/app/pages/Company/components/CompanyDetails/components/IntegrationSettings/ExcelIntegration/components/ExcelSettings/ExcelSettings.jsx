import styles from './ExcelSettings.module.scss';
...
function ExcelSettings({ companyID }) {
  const { salesforceSettings } = useIntegrationSettings(companyID);
  const handleCopy = (e) => {
    ...
    <div className={styles.salesforceSettings}>
      ...
      <div className={`${styles.rowValue} ${styles.connected}`}>
        {!salesforceSettings?.User?.Google_Sheets_Token?.is_logged_out ? (
          ...
          <h2 className={styles.notConnected}>Not Connected to Excel</h2>
        )}
      </div>
      ...
      <div className={`${styles.rowValue} ${styles.administrator}`}>
        ...
        <h3>
          Activated on{' '}
          {moment(salesforceSettings?.User?.created_at).format('DD MMM YYYY')}
        </h3>
      </div>
    </div>
  );
