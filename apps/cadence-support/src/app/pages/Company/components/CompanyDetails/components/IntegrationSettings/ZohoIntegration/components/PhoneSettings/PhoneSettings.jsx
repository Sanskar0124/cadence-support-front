...
import styles1 from '../FieldMappings/FieldMappings.module.scss';
...
function PhoneSettings({ companyID }) {
  const { phoneSystem, isPhoneSystemLoading } = usePhoneSystem(companyID);
  ...
  return (
    <>
      {isPhoneSystemLoading || isActivityLogsLoading ? (
        <div className={styles1.linePlaceholders}>
          {[...Array(2).keys()].map((key) => (
            <Skeleton className={styles1.linePlaceholder} />
          ))}
        </div>
      ) : (
        <div className={styles.phoneSettings}>
          ...
          <div className={styles.rowTitle}>
            <h1>Sync your SMS activities with Zoho</h1>
          </div>
          <div className={styles.rowValue}>
