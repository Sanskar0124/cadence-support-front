...
function TaskSettings({
  ...
  const { webhookSettings, webhookSettingsLoading } = useWebhook(companyID);
  ...
  useEffect(() => {
    ...
    if (selectedInnerTab === 'Webhook') {
      webhookRef.current.scrollIntoView({
        behavior: 'smooth',
      });
    }
  }, [selectedInnerTab]);
  ...
  return (
    <div className={styles.TaskSettings}>
      ...
      <div className={styles.leadscoring} ref={leadscoreRef}>
        ...
        {Lead_Score_Settings?.status_update && (
          <div className={styles.statusupdate}>
            <h2> Status Update</h2>
            {getStatusUpdate(Lead_Score_Settings)?.map((item) => (
        ...
        {isEmailSettingsLoading && (
          <Div loading={isEmailSettingsLoading} className={styles.loader} />
        )}
      ...
      {isWorkflowSettingsLoading && (
        <Div loading={isWorkflowSettingsLoading} className={styles.loader} />
      )}
      ...
      {webhookSettingsLoading && (
        <Div loading={webhookSettingsLoading} className={styles.loader} />
      )}

      <div className={styles.webhook} ref={webhookRef}>
        <h1>Webhook</h1>
        <h2>
          Custom webhooks in order to disqualify and convert directly from
          Cadence
        </h2>

        <div className={styles.customWebhook}>
          <h3>Your custom webhook</h3>
          <div className={`${styles.webhookDisplay}`}>
  ...
  return (
    <div className={styles.singleexception}>
      ...
      {exception?.status_update && (
        <div className={styles.statusupdate}>
          <h2> Status Update</h2>
          {getStatusUpdate(exception)?.map((item) => (
