...
import { WEEKDAYS, getunsuscribeList } from './constant';
...
function EmailSettings({
  ...
  useEffect(() => {
    setLoading(isEmailSettingsLoading);
  }, [isEmailSettingsLoading]);
  //  USEEFFECT FOR SCROLLING TO THE TARGET DIV

  useEffect(() => {
    if (selectedInnerTab === 'Unsubscribe rules') {
      unbouncedRuleRef.current.scrollIntoView({
        behavior: 'smooth',
      });
    }
    if (selectedInnerTab === 'Sending calendar') {
      sendingCalenderRef.current.scrollIntoView({
        behavior: 'smooth',
      });
    }
    if (selectedInnerTab === 'Bounced rules') {
      BouncedMailRules.current.scrollIntoView({
        behavior: 'smooth',
      });
    }
    if (selectedInnerTab === 'Domain verification') {
      domainverification.current.scrollIntoView({
        behavior: 'smooth',
      });
    }
  }, [selectedInnerTab]);
