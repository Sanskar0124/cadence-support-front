...
function HubspotSettings({ companyID }) {
      ...
      {isSalesforceSettingsLoading ? (
        ...
        <div className={styles.salesforceSettings}>
          ...
          <div className={`${styles.rowValue} ${styles.connected}`}>
            {!salesforceSettings?.User?.Hubspot_Token?.is_logged_out ? (
              <>
                <h2>
                ...
                <h3>
                  Activated on{' '}
                  {moment(
                    salesforceSettings?.User?.Hubspot_Token?.created_at
                  ).format('DD MMM YYYY')}
                </h3>
              </>
            ) : (
              <h2 className={styles.notConnected}>Not Connected to Hubspot</h2>
            )}
          </div>

          <div className={styles.rowTitle}>
            <h1>Hubspot Package</h1>
          </div>
          <div
            className={`${styles.rowValue} ${
              salesforceSettings?.User?.Hubspot_Token?.instance_url
                ? styles.url
                : ''
            }`}
          >
            {salesforceSettings?.User?.Hubspot_Token?.instance_url && (
              <>
                <h2>{salesforceSettings?.User?.Hubspot_Token?.instance_url}</h2>
                <button
                  onClick={() =>
                    handleCopy(
                      salesforceSettings?.User?.Hubspot_Token?.instance_url
